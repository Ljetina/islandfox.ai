...
* **Using Jupyter interface:** Jupyter Labs (the UI) doesn't reload the notebook from disk automatically. You unfortunately have to hit refresh on the browser to see the update made by the assistant. We prefer using [Visual Studio Code](https://code.visualstudio.com/) as the user interface for notebooks for this reason.
* **Keep the Notebook Compact:** Notebooks can get large quickly and the cost of reading it on every request runs up. For effective use of the chat assistant, keep your notebooks compact. For larger analysis, it can help to do multiple smaller steps in separate notebooks.
